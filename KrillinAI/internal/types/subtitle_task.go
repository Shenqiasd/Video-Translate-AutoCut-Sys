...
var SummaryAndTitlePrompt = `你是一个专业的视频内容总结专家。请根据提供的视频标题和描述，生成一个中文标题和一份约200字的中文视频简介。

要求如下：
1. **中文标题**：精炼、吸引人，准确概括视频内容。
2. **中文简介**：约200字，总结视频的核心内容、亮点和关键信息，语言通顺流畅。
3. **输出格式**：严格使用 "####" 分隔标题和简介，即：[中文标题]####[中文简介]

以下是视频的原始标题和描述（用####分隔）：
%s
`

var SummaryTranscriptPrompt = `你是一个专业的视频内容总结专家。请根据提供的视频字幕内容，生成一个中文标题和一份约200字的中文视频简介。
...
var BatchTranslatePrompt = `You are a professional subtitle translation expert.

[BATCH TRANSLATION TASK]
**Objective**:
Translate a list of subtitles into %s.

**Input**:
A JSON array of strings, where each string is a subtitle line. 
Example: ["Hello world", "This is a test"]

**Output Requirements**:
1. Return strictly a JSON array of strings.
2. The number of returned strings MUST match the input exactly.
3. Maintain the original order.
4. Do not include any explanation, markdown, or extra keys. Just the array.
5. Example output: ["你好世界", "这是一个测试"]
...
type SubtitleTaskStepParam struct {
	...
	TaskBasePath                string
	Link                        string
	AudioDownloadUrl            string        // New: Optional separate audio file URL to download
...
type SubtitleTask struct {
	...
	StatusMsg             string         `json:"status_msg" gorm:"column:status_msg"`                   // 详细状态描述
	...
	TtsVoiceCode          string         `json:"tts_voice_code" gorm:"column:tts_voice_code"`           // New: Persist voice selection for retry
	CreateTime            int64          `json:"create_time" gorm:"column:create_time;autoCreateTime"`  // 创建时间

...
func (s Service) linkToFile(ctx context.Context, stepParam *types.SubtitleTaskStepParam) error {
	...
	if strings.Contains(link, "local:") {
	...
	} else if stepParam.AudioDownloadUrl != "" {
		// New Feature: Separate audio file provided (uploaded)
		// Download audio directly from OSS/local URL
		log.GetLogger().Info("linkToFile: using separate audio file", zap.String("url", stepParam.AudioDownloadUrl))
		
		if strings.HasPrefix(stepParam.AudioDownloadUrl, "local:") {
			// Local file copy
			srcAudio := strings.ReplaceAll(stepParam.AudioDownloadUrl, "local:", "")
			err = util.CopyFile(srcAudio, audioPath)
			if err != nil {
				return fmt.Errorf("linkToFile copy local audio error: %w", err)
			}
		} else {
			// OSS download (if implemented) or skip for now assuming local: format for direct upload
			// For now, uploaded files in this system use "local:" prefix after saving
			// If it's a remote URL, we might need download logic. 
			// Assuming "local:" based on current upload handler.
			return fmt.Errorf("linkToFile: unsupported audio url format: %s", stepParam.AudioDownloadUrl)
		}
		...
		// Update status: Audio extraction starting
		stepParam.TaskPtr.StatusMsg = "正在提取音频 Extracting Audio..."
		...
		// Use absolute path for cookies and add verbose for debugging
		cmdArgs = append(cmdArgs, "--cookies", "/app/cookies.txt", "--verbose")
		...
		log.GetLogger().Info("DEBUG_V3: Executing yt-dlp command", zap.String("command", cmd.String()))
		output, err = cmd.CombinedOutput()
		if err != nil {
			log.GetLogger().Error("DEBUG_V3: linkToFile download audio yt-dlp error", zap.Any("step param", stepParam), zap.String("output", string(output)), zap.Error(err))
	...
	if !strings.HasPrefix(link, "local:") && stepParam.EmbedSubtitleVideoType != "none" {
		// Update status: Video download starting
		stepParam.TaskPtr.StatusMsg = "正在下载视频 Downloading Video..."
		_ = storage.SaveTask(stepParam.TaskPtr)
		...
		// FIX: Add cookies for video download too
		cmdArgs = append(cmdArgs, "--cookies", "/app/cookies.txt", "--verbose")
		...
		cmd := exec.Command(storage.YtdlpPath, cmdArgs...)
		log.GetLogger().Info("DEBUG_V3: Executing yt-dlp video command", zap.String("command", cmd.String()))
		output, err = cmd.CombinedOutput()
		if err != nil {

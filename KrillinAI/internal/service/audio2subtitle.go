...
// formatDuration 将时间格式化为易读的字符串（如 "5分30秒" 或 "2小时15分"）
func formatDuration(d time.Duration) string {
	...
	if hours > 0 {
		return fmt.Sprintf("%d小时%d分 / %dh%dm", hours, minutes, hours, minutes)
	} else if minutes > 0 {
		return fmt.Sprintf("%d分%d秒 / %dm%ds", minutes, seconds, minutes, seconds)
	}
	return fmt.Sprintf("%d秒 / %ds", seconds, seconds)
}
...
func (s Service) splitTextAndTranslateV2(basePath, inputText string, originLang, targetLang types.StandardLanguageCode, enableModalFilter bool, id int) ([]*TranslatedItem, error) {
	...
	var (
		...
		completedBatches int32 // 原子计数器
		// errChan = make(chan error, 1)
		// mutex   sync.Mutex
	)

	batchSize := 20
	totalBatches := (len(sentences) + batchSize - 1) / batchSize // 向上取整
	...
	for i := 0; i < len(sentences); i += batchSize {
		...
		batchSentences := sentences[i:end]
		wg.Add(1)
		signal <- struct{}{}

		go func(startIndex int, batch []string, batchNum int) {
			defer wg.Done()
			defer func() { <-signal }()
			...
			prompt := fmt.Sprintf(types.BatchTranslatePrompt, types.GetStandardLanguageName(targetLang), inputStr)

			var translatedBatch []string
			var err error

			// Retry logic for the batch
			for attempt := 0; attempt < config.Conf.App.TranslateMaxAttempts; attempt++ {
				var responseText string
				responseText, err = s.ChatCompleter.ChatCompletion(prompt)
				if err == nil {
					// 尝试解析JSON
					// 有时候LLM会返回 markdown 代码块，需要去掉 `json ... `
					...
					err = json.Unmarshal([]byte(cleanText), &translatedBatch)
					if err == nil {
						if len(translatedBatch) == len(batch) {
							break // Success
						}
						err = fmt.Errorf("batch size mismatch: expected %d, got %d", len(batch), len(translatedBatch))
					}
				}
				log.GetLogger().Warn("batch translate retry", zap.Int("attempt", attempt+1), zap.Error(err))
				time.Sleep(time.Duration(attempt+1) * time.Second)
			}

			// 更新批次完成计数
			completed := atomic.AddInt32(&completedBatches, 1)
			log.GetLogger().Info("Batch translation progress",
				zap.Int("splitId", id),
				zap.Int32("completed", completed),
				zap.Int("total", totalBatches),
				zap.String("progress", fmt.Sprintf("%d/%d", completed, totalBatches)))

			// Fill results
			for j, originText := range batch {
				globalIndex := startIndex + j
				if err != nil || len(translatedBatch) <= j {
					// Fallback to original text on failure
					if err != nil {
						log.GetLogger().Error("batch translate final error", zap.Int("batchIndex", startIndex), zap.Error(err))
					}
					results[globalIndex] = &TranslatedItem{
					...
					results[globalIndex] = &TranslatedItem{
						OriginText:     originText,
						TranslatedText: translatedBatch[j],
					}
				}
			}
		}(i, batchSentences, i/batchSize)
	}
	...
	for range config.Conf.App.TranscribeParallelNum {
		eg.Go(func() error {
			for {
				...
				case <-ctx.Done():
					return nil
				case item, ok := <-pendingTranscriptionQueue:
					if !ok {
						return nil
					}

					// Resume Capability: Check if transcription already exists
					transcriptionSavePath := filepath.Join(stepParam.TaskBasePath, fmt.Sprintf(types.SubtitleTaskAudioTranscriptionDataPersistenceFileNamePattern, item.Id))
					var transcriptionData *types.TranscriptionData

					// Try load existing data
					fileBytes, err := os.ReadFile(transcriptionSavePath)
					if err == nil {
						if json.Unmarshal(fileBytes, &transcriptionData) == nil && transcriptionData != nil && transcriptionData.Text != "" {
							log.GetLogger().Info("Resume Capability: Found existing transcription data, skipping Whisper", zap.String("path", transcriptionSavePath))
							// Directly send to transcribedQueue
					...
					// If no valid existing data, proceed with Whisper
					log.GetLogger().Info("Begin to transcribe", zap.Any("taskId", stepParam.TaskId), zap.Any("splitId", item.Id))
					for range config.Conf.App.TranscribeMaxAttempts {
						transcriptionData, err = s.transcribeAudio(item.Id, item.Data, string(stepParam.OriginLanguage), stepParam.TaskBasePath)
						if err == nil {
							break
						}
					}
					if err != nil {
						return fmt.Errorf("audioToSubtitle audioToSrt transcribeAudio err: %w", err)
					}
					log.GetLogger().Info("Transcribe completed", zap.Any("taskId", stepParam.TaskId), zap.Any("splitId", item.Id))
					transcribedQueue <- DataWithId[*types.TranscriptionData]{
						Data: transcriptionData,
						Id:   item.Id,
					}
	...
	eg.Go(func() error {
		for {
			...
			case translateItem, ok := <-pendingTranslationQueue:
				...
				// Resume Capability: Check if translation already exists
				translationSavePath := filepath.Join(stepParam.TaskBasePath, fmt.Sprintf(types.SubtitleTaskTranslationDataPersistenceFileNamePattern, translateItem.Id))
				var translatedResults []*TranslatedItem
				var err error

				loadedData, loadErr := util.LoadFromDisk(translationSavePath)
				if loadErr == nil {
					// Found existing translation data
					log.GetLogger().Info("Resume Capability: Found existing translation data, validating...", zap.String("path", translationSavePath))
					// Improve robustness: Read file to ensure valid JSON
					fileBytes, _ := os.ReadFile(translationSavePath)
					if json.Unmarshal(fileBytes, &translatedResults) == nil {
						// Validate content: Check for untranslated segments (fallback to origin)
						// access attempt loop or check identical text if languages differ
						if stepParam.TargetLanguage != "none" && stepParam.TargetLanguage != stepParam.OriginLanguage {
							invalidCount := 0
							for _, item := range translatedResults {
								// Heuristic: If text is identical and contains letters (to avoid "123" or "?"), mark as suspicious
								// Simplified: Just check identical for now.
								if item.OriginText == item.TranslatedText && len(strings.TrimSpace(item.OriginText)) > 0 {
									invalidCount++
								}
							}
				...
				if loadedData == nil {
					// No valid existing data, perform translation
					...
					// Persist translation result
					_ = util.SaveToDisk(translatedResults, translationSavePath)
	...
	eg.Go(func() error {
		...
		for {
			...
			case splitResultItem := <-splitResultQueue:
				...
				_ = storage.SaveTask(stepParam.TaskPtr) // Persist progress to DB
				// 处理分割结果
			...
			case translatedItems := <-translatedQueue:
				...
				elapsed := time.Since(translateStartTime)
				if completedTranslations > 0 {
					avgTimePerTranslation := elapsed / time.Duration(completedTranslations)
					remaining := totalTranslations - completedTranslations
					eta := avgTimePerTranslation * time.Duration(remaining)
					etaStr := formatDuration(eta)
					stepParam.TaskPtr.StatusMsg = fmt.Sprintf("正在翻译 Translating... (%d/%d) 预计剩余 ETA: %s", completedTranslations, totalTranslations, etaStr)
					log.GetLogger().Info("Translation progress",
						zap.Int("completed", completedTranslations),
						zap.Int("total", totalTranslations),
...
func splitSrt(stepParam *types.SubtitleTaskStepParam) error {
	...
	// 供生成配音使用
	// 供生成配音使用 - 优先使用目标语言字幕，实现配音
	if stepParam.TargetLanguage != "none" {
	...
	if _, err := os.Stat(originSummaryPath); err == nil {
		subtitleInfo = types.SubtitleFileInfo{
			Path:               originSummaryPath,
			LanguageIdentifier: string(stepParam.OriginLanguage),
		}
		if stepParam.UserUILanguage == types.LanguageNameEnglish {
			subtitleInfo.Name = types.GetStandardLanguageName(stepParam.OriginLanguage) + " Summary"
		} else if stepParam.UserUILanguage == types.LanguageNameSimplifiedChinese {
			subtitleInfo.Name = types.GetStandardLanguageName(stepParam.OriginLanguage) + " 视频摘要"
		}
		stepParam.SubtitleInfos = append(stepParam.SubtitleInfos, subtitleInfo)
	}
	...
	if _, err := os.Stat(targetSummaryPath); err == nil {
		subtitleInfo = types.SubtitleFileInfo{
			Path:               targetSummaryPath,
			LanguageIdentifier: string(stepParam.TargetLanguage),
		}
		if stepParam.UserUILanguage == types.LanguageNameEnglish {
			subtitleInfo.Name = types.GetStandardLanguageName(stepParam.TargetLanguage) + " Summary"
		} else if stepParam.UserUILanguage == types.LanguageNameSimplifiedChinese {
			subtitleInfo.Name = types.GetStandardLanguageName(stepParam.TargetLanguage) + " 视频摘要"
		}
		stepParam.SubtitleInfos = append(stepParam.SubtitleInfos, subtitleInfo)
	}
...
func generateSrtWithTimestamps(srtBlocks []*util.SrtBlock, tsOffset float64, words []types.Word, segmentIdx int, stepParam *types.SubtitleTaskStepParam) error {
	...
	log.GetLogger().Info("DEBUG_SUB: generateSrtWithTimestamps", zap.Int("segmentIdx", segmentIdx), zap.Float64("tsOffset", tsOffset), zap.Int("numBlocks", len(srtBlocks)))
